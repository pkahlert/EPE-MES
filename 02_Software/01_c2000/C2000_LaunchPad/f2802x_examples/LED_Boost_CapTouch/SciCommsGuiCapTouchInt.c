...
// Note: Assumes GPIO pins for SCIA are configured to Primary function    
    ...
    RcvTaskPointer = &CmdInterpreter;            // Initialize the CmdPacket Rcv Handler state machine ptr
...
void SciISR(void) // Task 1
{
    ...
    else if (SciaRegs.SCIRXST.bit.BRKDT == 1 || SerialCommsTimer > 30) //~2 s timeout
    {    
...
void CmdInterpreter(void) // Task 3
{
...
void CmdInspector(void)
{
    ...
    }else if((RxChar & 0x0020) == 0x0020)
    {
        ...
        if(RxChar & 0x8000)
        {
            ...
            gHSV.qHue = gHSV.qHue - (_IQ15mpy(_IQ15((RxChar & 0x7F00) >> 8), _IQ15(10)));
