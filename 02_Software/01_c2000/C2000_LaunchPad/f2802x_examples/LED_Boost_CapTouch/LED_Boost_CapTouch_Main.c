...
//!   spinning their finger on the capacitive touch boosterpack when plugged into
//!   the LED boosterpack.
//!
...
//!   BoosterPack directly and reports actions to the Piccolo device on the C2000
//!   LaunchPad via an asynchronous serial interface.
//!
//!   After loading and running this example, press the center section of the
//!   capacitive touch BoosterPack twice to turn on the LEDs.  Moving ones finger
//!   around the outer sections of the touchpad will change the LED color.
//!   Touching the center section again will turn off the LEDs.
//
...
int16    VTimer0[4]={0,0,0,0};         // Virtual Timers slaved off CPU Timer 0 (A events)
int16    VTimer1[4]={0,0,0,0};         // Virtual Timers slaved off CPU Timer 1 (B events)
int16    VTimer2[4]={0,0,0,0};         // Virtual Timers slaved off CPU Timer 2 (C events)
...
int16    ResetSlew=0;             // Forces update of individual SlewSteps with SlewStepAll
...
//=================================================================================
//--------------------------------------------------------
void A1(void) // Over-current protection, On/Off Control 
//--------------------------------------------------------
{
...
//-----------------------------------------------------------------
void A2(void) // SCI GUI, Slew control, LED current control
//-----------------------------------------------------------------
{
    SerialHostComms();    //found in SciCommsGui.c

    // This code ensures that each channel's current ramps at a given slew rate at a rate defined by SlewStep.
    ...
    // Iset = Gui_Iset * iK_Iout, where iK_Iout = 1/K_Iout (i.e. inverse K_Iout)
    // view and set following variables in Watch Window as:
    //        Gui_Iset = Q15
...
//-----------------------------------------------------------------
void comp(void) // Hardware fix for poor feedback path on LED Boosterpack
//-----------------------------------------------------------------
{
...
//----------------------------------------
void C1(void) //  Blinking LED control
//----------------------------------------
{
...
//----------------------------------------
void C2(void) //  Automatic color control, Manual color control
//----------------------------------------
{
...
void HSVtoRGB(tHSV *myHSV, tRGB *myRGB)
{
    ...
    qX = _IQ15mpy(qChroma, _IQ15(1) - _IQ15abs(_IQ15(lHPrimeInt % 2) + qHPrimeFrac - _IQ15(1)));
